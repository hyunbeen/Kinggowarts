(function ()
{
    ...
    /** @ngInject */
    function MapController($mdDialog, CustomEventData, DrawingMenuData, SubAreaData, CategoryMarkerData, MarkerData, $scope, $interval, $timeout, peerLocation, mapLocation)
    {
        var vm = this;
...
/*
    ...
    //드로잉 관련 기타 함수
        ...
        function removeOverlayAndGetData(inObjData){
            ...
            newCustomEventObj["lat"] = curDrawingObj["marker"][0]["y"];
            newCustomEventObj["lng"] = curDrawingObj["marker"][0]["x"];
        ...
        function addDataToDrawingManager(data){
            ...
            if(data["shape"] == "CIRCLE"){
                ...
                var bounds = new daum.maps.LatLngBounds(
                    new daum.maps.LatLng(data["rectangleSPointLat"], data["rectangleSPointLng"]),
                    new daum.maps.LatLng(data["rectangleEPointLat"], data["rectangleEPointLng"])
                    );
...
//------controller scope 밖 function
    ...
    //마커가 drawingObj, shapeType에 해당하는 도형 안에 존재하는지 확인. (원은 수정 더 필요)
